...
export interface ExpenseParticipant {
  memberId: string
  weight: number // share weight; 1 = equal share
}
...
class FinanceDB extends Dexie {
  groups!: Table<FinanceGroup, string>
...
export async function createGroup(
  input: Omit<FinanceGroup, 'id' | 'createdAt'> & { id?: string },
) {
...
export async function listExpensesByGroup(groupId: string) {
...
export function computeBalances(group: FinanceGroup, expenses: Expense[]) {
  ...
  for (const exp of expenses) {
    ...
    const totalWeight =
      exp.participants.reduce((s, p) => s + (p.weight || 0), 0) || 1
...
export function suggestSettlements(
  ...
  while (i < debtors.length && j < creditors.length) {
    ...
    if (pay > 0) transfers.push({ from: d.id, to: c.id, amountCents: Math.round(pay) })
    d.amount -= pay
    c.amount -= pay
    if (d.amount === 0) i++
    if (c.amount === 0) j++
...
// ---- Utils ----
export function normalizeParticipants(parts: ExpenseParticipant[]): ExpenseParticipant[] {
  // remove zero/negative weights, collapse duplicates, default weight=1
  const acc = new Map<string, number>()
  for (const p of parts) {
    const w = p.weight ?? 1
    if (w > 0) acc.set(p.memberId, (acc.get(p.memberId) ?? 0) + w)
  }
  const out = Array.from(acc.entries()).map(([memberId, weight]) => ({ memberId, weight }))
  return out.length ? out : []
}

export function formatCents(amountCents: number, currency: CurrencyCode) {
